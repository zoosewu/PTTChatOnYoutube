...
export function InitApp(chatcon, whitetheme, isstreaming, messageposter) {
  ...
  function InitChatApp(cn) {
    ...
    let PTT = new Vue({
      ...
      data: function () {
        return {
          ...
          playertime: window.setInterval((() => {
            if (this.player) {
              console.log("player: ", this.player, this);
              console.log("player time: ", this.player.currentTime);
    ...
    const PTTAppx = $(`<div id="PTTChat-app" class=" pttbg border rounded w-100 d-flex flex-column"></div>`);
