import { showcommand, reportmode, showalllog, showPttscreen } from '../logsetting'
let cryptkey
export function InitPTT (messageposter) {
  ...
  function updatePagestate () {
    for (let i = 0; i < PTT.pagestatefilter.length; i++) {
      ...
      if (result != null) {
        if (reportmode) {
          console.log(
            '==page state:' + PTT.pagestate + '->' + filter.state,
            result
          )
        }
  ...
  function OnUpdate () {
    ...
    if (showcommand && typeof nextcom !== 'undefined') {
      console.log(
        '==next command : reg:' + nextcom.reg + 'input:' + nextcom.input,
        [nextcom.callback]
      )
  ...
  function boardcheck () {
    ...
    if (PTT.pagestate === 4 || PTT.pagestate === 3) {
      ...
      reg =
        '看板《' +
        (PTTPost.enableautofetchpost
          ? PTTPost.searchingTitle.boardforsearch
          : PTTPost.board) +
        '》'
      re =
        '系列《' +
    ...
    const curr = PTT.screenHaveText(re)
  ...
  function gotoPost () {
    if (!PTTPost.enableautofetchpost) {
      if (PTT.screenHaveText(/系列《.+》/)) insertText('q')
      // 關鍵字搜尋狀態不能#AID
  ...
  function PostCheck () {
    ...
    if (PTT.pagestate === 2) {
      if (
        (PTTPost.enableautofetchpost && !PTTPost.enteredTitle) ||
        (!PTTPost.enableautofetchpost && !PTTPost.enteredTitle)
      ) { res.pass = false }
      if (
        (!PTTPost.enableautofetchpost && PTTPost.enteredAID) ||
        (PTTPost.enableautofetchpost && PTTPost.enteredTitle)
      ) {
        if (
          PTT.screenHaveText(
          ...
          msg.PostMessage('alert', {
            type: 0,
            msg: '文章AID錯誤，文章已消失或是你找錯看板了。'
          })
          if (reportmode) {
            console.log(
              '文章AID錯誤，文章已消失或是你找錯看板了',
              PTT.pagestate,
              PTT,
              PTTPost
            )
  ...
  function PotsTitleCheck () {
    ...
    if (PTT.pagestate === 3) {
      ...
      if (posttitle) {
        ...
        for (let i = 0; i < 5 && i < PTT.screen.length; i++) { title += PTT.screen[i] }
      } // 抓前幾行
      if (PTTPost.samepost) {
        if (!PTTPost.enableautofetchpost) {
    ...
    } else if (PTT.pagestate === 1) { console.log('==PotsTitleCheck error, PTT.pagestate == 1.') } else if (PTT.pagestate === 2) { console.log('==PotsTitleCheck error, PTT.pagestate == 2.') }
    return res
  }
  ...
  function PostLineCheck () {
    ...
    if (PTT.pagestate === 4 || PTT.pagestate === 3) {
      ...
      if (
        (targetline < 1 || targetline > 23) &&
        PTT.screenHaveText(
          /瀏覽 第 \d+\/\d+ 頁 \(100%\) +目前顯示: 第 \d+~\d+ 行/
        ) === null
      ) { res.pass = false } else getpush()
    } else if (PTT.pagestate === 1) { console.log('==PistLineCheck error, PTT.pagestate == 1.') } else if (PTT.pagestate === 2) { console.log('==PistLineCheck error, PTT.pagestate == 2.') }
    return res
  }
  ...
  function savepush (content, result) {
    ...
    pushdata.date = new Date(
      PTTPost.posttime.getFullYear(),
      result[4] - 1,
      result[5],
      result[6],
      result[7]
  ...
  function getpush () {
    ...
    for (let i = targetline; i < PTT.screen.length; i++) {
      ...
      const result = /^(→ |推 |噓 )(.+?): (.*)(\d\d)\/(\d\d) (\d\d):(\d\d)/.exec(
        line
      )
      if (result != null) {
  ...
  function PostPercentCheck () {
    ...
    if (
      (PTT.pagestate === 3 || PTT.pagestate === 4) &&
      PTT.screenHaveText(
        /瀏覽 第 \d+\/\d+ 頁 \(100%\) +目前顯示: 第 \d+~\d+ 行/
      ) !== null
    ) {
      res.pass = true
    } else if (PTT.pagestate === 1) { console.log('==PostPercentCheck error, PTT.pagestate == 1.') } else if (PTT.pagestate === 2) { console.log('==PostPercentCheck error, PTT.pagestate == 2.') }
    return res
  }
  ...
  function GetRecentLine () {
    ...
    } else if (PTT.pagestate === 1) { console.log('==GetPushTask error, PTT.pagestate == 1.') } else if (PTT.pagestate === 2) { console.log('==GetPushTask error, PTT.pagestate == 2.') }
    return res
  }
  //
  // -----------------------task setNewPush --------------------
  ...
  function SetNewPush () {
    ...
    SetNewPushtrytime--
    if (SetNewPushtrytime < 0) {
      res.pass = true
      return res
    }
    if (PTT.pagestate === 4 || PTT.pagestate === 3) {
      const pushcd = PTT.screenHaveText(
  ...
  function CheckTitleSame (_boardforsearch, _titleforsearch, task) {
    ...
    if (PTT.pagestate === 1) {
      if (PTT.screenHaveText(/(> |●)\(M\)ail {9}【 私人信件區 】/)) {
        insertText('c')// 隨意切畫面
  ...
  function GetRecentLineTask () {
    RunTask(task.GetPostRecentLine, () =>
      PTT.commands.add(/.*/, '', GetPushTask)
    )
  }
  function GetPushTask () {
    RunTask(task.GetPostByLine, recievePushes)
  }
  ...
  function GetPush (
    ...
    const auto =
      pAID === undefined &&
      bname === undefined &&
      pboardforsearch === undefined &&
      ptitleforsearch === undefined
    const samepostbyAID =
      bname === PTTPost.board &&
      pAID === PTTPost.AID &&
      pAID !== undefined &&
      bname !== undefined
    const samepostbytitle =
      pboardforsearch === PTTPost.searchingTitle.boardforsearch &&
      ptitleforsearch === PTTPost.searchingTitle.titleforsearch
    ...
    } else if (PTT.pagestate === 2) insertText('P')
    // 切下一頁
    else {
      // PTT.pagestate === 3 || 4
      if (!PTTPost.samepost) {
    ...
    if (!PTT.login) {
      ...
      const logincheck = () => {
        if (
          ...
          PTT.screenHaveText(/上方為使用者心情點播留言區|【 精華公佈欄 】/)
        ) {
          ...
          PTT.screenHaveText(
            /登入中，請稍候\.\.\.|正在更新與同步線上使用者及好友名單，系統負荷量大時會需時較久|密碼正確！ 開始登入系統/
          )
        ) {
  ...
  msg.login = data => {
    const i = CryptoJS.AES.decrypt(data.id, cryptkey).toString(
      CryptoJS.enc.Utf8
    )
    const p = CryptoJS.AES.decrypt(data.pw, cryptkey).toString(
      CryptoJS.enc.Utf8
    )
    TryLogin = 2
    // console.log(data );
  ...
  msg.getPushByRecent = data => {
    if (reportmode) console.log('getPushByRecent', data)
    PTTLockCheck(
      CheckLoginState,
      GetPush,
      data.AID,
  ...
  msg.setNewPush = data => {
    if (reportmode) console.log('setNewPush', data)
    PTTLockCheck(SetNewPushTask, data)
  }
  msg.getPostTitle = data => {
    if (reportmode) console.log('getPostTitle', data)
    PTTLockCheck(
      CheckLoginState,
      CheckTitleSame,
      data.boardforsearch,
