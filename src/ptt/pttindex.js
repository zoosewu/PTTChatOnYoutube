...
export function InitPTT (messageposter) {
  ...
  function boardcheck () {
    ...
    if (PTT.pagestate === 4 || PTT.pagestate === 3) {
      ...
      reg = '看板《' + (PTTPost.titleSearch ? PTTPost.titleSearch.board : PTTPost.board) + '》'
      re = '系列《' + (PTTPost.titleSearch ? PTTPost.titleSearch.board : PTTPost.board) + '》'
    }
  ...
  function PotsTitleCheck () {
    ...
    if (PTT.pagestate === 3) {
      ...
      if (PTTPost.samepost) {
        ...
        PTTPost.title = title
        if (PTTPost.titleSearch) msg.PostMessage('setPreviewPostTitle', PTTPost.title)
  ...
  //   if (PTT.pagestate === 2) {
  //     if (!PTTPost.titleSearch.isend.insertP || !PTTPost.titleSearch.isend.insert$) res.pass = false
  //     else {
  //       const reg = /^(>|●).+(□|R:|轉)/
  //       const posttitle = PTT.screenHaveText(reg)
  //       let title = ''
  ...
  //       else PTTPost.titleSearch.fullTitle = title
  //     }
  //   }
  //   return res
  // }
  ...
  function GetPush (pAID, bname, startline, task, ptitleSearch) {
    ...
    const sameAID = Boolean(PTTPost.board) && Boolean(PTTPost.AID) && (bname === PTTPost.board) && (pAID === PTTPost.AID)
    const sameTitle = Boolean(PTTPost.titleSearch) && Boolean(ptitleSearch) && (ptitleSearch.title === PTTPost.title)
  ...
  msg.getPushByRecent = data => { if (reportmode) console.log('getPushByRecent', data); PTTLockCheck(CheckLoginState, GetPush, data.AID, data.board, data.recent, GetRecentLineTask, data.titleSearch) }
