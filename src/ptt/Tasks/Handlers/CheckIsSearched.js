import { SearchType } from '../../MessagePosterData/PostData.js'
...
const gotoPost = () => {
  this.postData.isPostChecked = true
  if (this.postData.searchType === SearchType.AID) {
    gotoPostByAID()
  } else if (this.postData.searchType === SearchType.Title) {
    gotoPostByTitle()
  }
}
...
export default function CheckIsInPost () {
  ...
  if (this.state.frame === FrameState.board) {
    if (this.postData.isPostChecked) {
      ...
      if (result) {
        ...
        if (reportMode) console.log(message, this, this.postData)
        res.pass = false
        this.unlock()
      }
    }
